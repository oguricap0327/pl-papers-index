...
## [A Lazy, Concurrent Convertibility Checker](https://arxiv.org/pdf/2510.18418)

**Authors**: Nathanaëlle Courant, Xavier Leroy  
**Venue**: POPL 2026  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776XXX  

> Convertibility checking - determining whether two lambda-terms are equal up to reductions - is a crucial component of proof assistants and dependently-typed languages. This paper presents a novel convertibility-checking algorithm that relies crucially on laziness and concurrency. Laziness is used to share computations, while concurrency is used to explore multiple convertibility subproblems in parallel or via fair interleaving. Unlike heuristics-based approaches, the algorithm always finds an easy solution to the convertibility problem, if one exists.

**Keywords**: `convertibility checking`, `proof assistants`, `dependent types`, `concurrency`, `laziness`

**Related**:
- Related: type checking, dependent types, proof assistants

---

## [On Circuit Description Languages, Indexed Monads, and Resource Analysis](https://arxiv.org/pdf/2511.22419)

**Authors**: Ken Sakayori, Andrea Colledan, Ugo Dal Lago  
**Venue**: POPL 2026  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776XXX  

> This paper introduces a monad-based denotational model for the Proto-Quipper family of calculi, idealized versions of the Quipper quantum programming language. The monadic approach separates the value to which a term reduces from the circuit that the term produces as a side effect. This enables denotational interpretation and validation of rich type systems in which the size of the produced circuit can be controlled. The framework suggests forms of effect typing guaranteeing quantitative properties about the resulting circuit, even in presence of optimizations.

**Keywords**: `quantum computing`, `indexed monads`, `circuit description`, `resource analysis`, `effect types`

**Related**:
- Related: Quipper, quantum programming, effect systems

---

## [Handling Higher-Order Effectful Operations with Judgemental Monadic Laws](https://arxiv.org/pdf/2511.05739)
...
> This paper studies the design of programming languages with handlers of higher-order effectful operations - effectful operations that may take in computations as arguments or return computations as output. The distinctive design choice is that handlers are carried by lawless raw monads, while the computation judgements still satisfy the monadic laws judgementally. The paper presents a core calculus with higher-kinded impredicative polymorphism, handlers of higher-order effectful operations, and optionally general recursion, with denotational models using realizability semantics.

**Keywords**: `algebraic effects`, `effect handlers`, `higher-order effects`, `monads`, `type theory`

**Related**:
- Related: algebraic effects, effect handlers, monadic programming

---

## [Expanding Specification Capabilities of a Gradual Verifier with Pure Functions](https://arxiv.org/pdf/2511.22075)

**Authors**: Doruk Alp Mutlu  
**Venue**: POPL 2026 (Student Research Competition)  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776XXX  

> Gradual verification soundly combines static checking and dynamic checking to provide an incremental approach for software verification. This work explores an extension to Gradual C0's design with pure functions, which both extend the specification capabilities of Gradual C0 and increase the ease of encoding observer methods. The approach addresses the technical challenges related to the axiomatisation of pure functions with imprecise specifications.

**Keywords**: `gradual verification`, `program verification`, `pure functions`, `symbolic execution`

**Related**:
- Related: gradual typing, program verification, separation logic

---

## [The Ghosts of Empires: Extracting Modularity from Interleaving-Based Proofs](https://arxiv.org/pdf/2511.20369)

**Authors**: Frank Schüssele, Gidon Ernst, Heike Wehrheim  
**Venue**: POPL 2026  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776684  

> Implementation bugs threaten the soundness of algorithmic software verifiers. This paper presents an approach that converts an interleaving-based correctness proof, as generated by many algorithmic verifiers, into a thread-modular correctness proof in the style of Owicki and Gries. The approach automatically synthesizes ghost variables that capture the relevant interleaving information, and abstract away irrelevant details. The evaluation shows that the approach is efficient in practice and generates compact proofs.

**Keywords**: `program verification`, `concurrency`, `thread-modular proofs`, `ghost variables`, `Owicki-Gries`

**Related**:
- Related: concurrent program verification, separation logic, model checking

---

## [Characterizing Sets of Theories That Can Be Disjointly Combined](https://arxiv.org/pdf/2511.17374)

**Authors**: Benjamin Przybocki, Andrew Reynolds, Cesare Tinelli, Clark Barrett  
**Venue**: POPL 2026  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776XXX  

> This paper studies properties that allow first-order theories to be disjointly combined, including stable infiniteness, shininess, strong politeness, and gentleness. The authors describe a Galois connection between sets of decidable theories, which picks out the largest set of decidable theories that can be combined with a given set of decidable theories. This strengthens previous results and answers in the negative several long-standing open questions about the possibility of improving existing theory combination methods.

**Keywords**: `SMT solving`, `theory combination`, `first-order logic`, `decidability`, `Galois connection`

**Related**:
- Related: SMT solvers, automated reasoning, decision procedures

---

## [TypeDis: A Type System for Disentanglement](https://arxiv.org/pdf/2511.23358)

**Authors**: Alexandre Moine, Sam Westrick, Umut A. Acar  
**Venue**: POPL 2026  
**Year**: 2026  
**DOI**: https://doi.org/10.1145/3776655  

> Disentanglement is a runtime property of parallel programs guaranteeing that parallel tasks remain oblivious to each other's allocations. This paper explores automatic verification via a type system, ensuring that any well-typed program is disentangled. TypeDis is a type system inspired by region types, where each type is annotated with a timestamp, identifying the task that allocated it. TypeDis supports iso-recursive types as well as polymorphism over both types and timestamps. The soundness of TypeDis is mechanized in the Rocq proof assistant.

**Keywords**: `type systems`, `parallelism`, `memory management`, `garbage collection`, `region types`

**Related**:
- Related: region types, parallel programming, memory safety
